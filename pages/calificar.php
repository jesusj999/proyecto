...
// Verifica que el formulario fue enviado y que se seleccionaron grado, grupo y materia
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $grado_sel && $grupo_sel && $materia_sel) {

    // Busca todos los estudiantes activos del grupo seleccionado con sus notas actuales
    $estudiantes_cal = $conn->query("SELECT p.identificacion, p.primerNombre, p.primerApellido,
                                     ...
                                     -- Trae estudiantes de la tabla matricula
                                     FROM matricula m

                                     -- JOIN para obtener nombre y apellido del estudiante
                                     JOIN personas p ON m.idEstudiante = p.identificacion

                                     -- LEFT JOIN con nota: si el estudiante no tiene nota
                                     -- aún, igual aparece en la lista con los campos vacíos
                                     LEFT JOIN nota n ON (n.idEstudiante = p.identificacion
                                         AND n.idmateria = $materia_sel    -- Filtra por materia seleccionada
                                         AND n.idgrado = $grado_sel        -- Filtra por grado seleccionado
                                         AND n.num_informe = $informe_sel  -- Filtra por número de informe
                                         AND n.anio = $anio_sel)           -- Filtra por año seleccionado

                                     -- Solo trae estudiantes del grado y grupo seleccionado
                                     WHERE m.idGrado = $grado_sel
                                     AND m.idGrupo = $grupo_sel
                                     AND m.anio = $anio_sel

                                     -- idEstadoActual=1 significa solo estudiantes Activos
                                     AND m.idEstadoActual = 1

                                     -- Ordena la lista alfabéticamente por apellido y nombre
